 ...
 *
 * Run: node scripts/extract-legacy-components.mjs
 */
...
const VRT_PATH = path.join(__dirname, '../frontend/public/assets/vrt-all-components.html');
const CONTENT_DIR = path.join(__dirname, '../content');
const LEGACY_CATEGORY = 'legacy-slds1';
// Tab-based: legacy-components--accordion etc. (not nested legacy-components/accordion)
const LEGACY_COMPONENTS_DIR = path.join(CONTENT_DIR, LEGACY_CATEGORY);
...
function extractPreviewHtml(html, startIndex) {
...
function extractComponents(html) {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    while ((exMatch = exampleRe.exec(sectionHtml)) !== null) {
      const { html: previewHtml } = extractPreviewHtml(sectionHtml, exMatch.index);
      if (previewHtml) {
        examples.push({
          id: exMatch[1],
...
function main() {
  ...
  for (const comp of components) {
    const mdPath = path.join(LEGACY_COMPONENTS_DIR, `legacy-components--${comp.id}.md`);
