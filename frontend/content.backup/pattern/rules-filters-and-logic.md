...
category: "pattern"
url: "https://www.lightningdesignsystem.com/2e1ef8501/p/58ff5f-rules-filters-and-logic"
extractedAt: "2026-02-21T21:59:05.219Z"
---
...
Create intuitive interfaces for working with rules, filters, and logic—no matter how complex—using a thoughtful combination of Lightning Design System components.
...
[Open in Figma](https://www.figma.com/@salesforce)Salesforce enables users to store, sort, and take action on large, complex datasets. Whether creating rules to control when an action is executed or querying for a filtered set of records, logical expressions should be easy to read and manipulate. Giving users confidence while working with their data enables them to use the full capabilities of Salesforce to run their business.
...
Use the expression component to show a toggle for the logic mode and allow users to change it to any available option.Criteria
...
When users **are not **adding their own rules and **cannot** change the logic mode, use components such as combobox, dual listbox, select, and text input. These simple and familiar elements will give users fast access to their desired results.When users are not adding their own rules and cannot change the logic mode, use components such as combobox, dual listbox, select, and text input. These simple and familiar elements will give users fast access to their desired results.
