...
category: "general"
slug: "prompt-elements--context-setting"
status: "published"
...
| **Role or Persona** + **Situation + Key Task + End Goal***As a ________ when I’m ___________ I want to __________ so I can __________.* |
| --- |
| **Scenario 1: **Send an email<ul><li>As a **Sales Development Representative, **when I’m **reaching out to a new prospect**, I want to **create a personalized introduction email** so I can **get a response and initiate a conversation**.</li></ul> |
| **Scenario 2: **Summarize<ul><li>As a **Sales Manager, **when I’m **preparing for a client meeting**, I want to **summarize their recent interactions **so I can **have an informed discussion and build a stronger relationship.**</li></ul> |
...
| **Sub-Element** | **Function** | **Pattern Examples** |
| --- | --- | --- |
| Core Action | Use a strong action verb for the task that defines the LLM's operation. | <ul><li>Your task is to [action verb] ...</li><li>Your objective is to [action verb]...</li><li>[imperative action verb]...</li><li>You’re tasked with [action verb]...</li></ul> |
| Output Description | Describe the expected output object and potentially reference the format.Help the LLM organize the generated content correctly from the start. | <ul><li>...[output]...</li><li>...[output] in [format]...</li><li>...[format] [output]</li></ul> |
| Data Description | Include a description or location of the data or object that will be used to produce the output.Point the LLM to the specific information, documents, or knowledge sources it must use or focus on before beginning the task. | <ul><li>...from [data]...</li><li>...with the provided [data] below</li></ul> |
| Task Constraints | Highlight critical constraints upfront so the LLM considers them before its generation process. | <ul><li>...using only [constraints]</li><li>...looking at just [constraints]</li><li>...considering only [constraints]</li><li>...limited to [constraints]</li><li>...without any [excluded content]</li></ul> |
...
| **Patterns** | **Examples** |
| --- | --- |
| `[article]` + `[adjective]` + `[noun]``[article]` + `[noun]``[adjective]` + `[noun]` | <ul><li>a concise summary</li><li>an email</li><li>a detailed analysis</li><li>a bulleted list</li><li>sentiment classification</li></ul> |
| a draft of `[noun phrase]` | <ul><li>a draft of a sales proposal</li><li>a draft of a warm introduction email</li></ul> |
| `[noun phrase]` of `[content or topic]` | <ul><li>a list of action items</li><li>a numbered list of the top 3 opportunities</li><li>a concise summary of next steps</li><li>the classification of the condition</li></ul> |
| `[noun phrase]` of `[data description]` | <ul><li>a summary of a call transcript</li><li>a translation of the following text</li><li>a detailed analysis of the case below</li></ul> |
| `[noun phrase]` in `[format]``[noun phrase]` formatted in `[format]` | <ul><li>a summary in Markdown</li><li>a list of action items formatted in HTML</li></ul> |
...
| **Patterns** | **Examples** |
| --- | --- |
| Your task is to summarize `[A]`, `[B]`, and `[C]` in `[Data]`. | Your task is to summarize the overall impressions, key strengths, and growth opportunities mentioned in a presentation. |
| Your task is to extract information about `[A]`, `[B]`, and `[C]` from `[Data]`. | Your task is to extract information about customer impressions, call summary, and next steps from a call transcript. |
...
1. Why do you want to summarize a client's accounts?<ol><li>*...you can quickly review their financial status.*</li></ol>
2. *...you can quickly review their financial status.*
3. Why do you want to review their financial status?<ol><li>*...you can identify trends or issues for discussion.*</li></ol>
4. *...you can identify trends or issues for discussion.*
5. Why do you want to identify trends and issues?<ol><li>*...you can tailor recommendations to their situation.*</li></ol>
6. *...you can tailor recommendations to their situation.*
7. Why do you want to tailor recommendations?<ol><li>.*..you can show you understand their business and offer relevant insights.*</li></ol>
8. .*..you can show you understand their business and offer relevant insights.*
9. Why do you want to show you understand their business?<ol><li>*...you can build a stronger relationship with them.*</li></ol>
10. *...you can build a stronger relationship with them.*


Why do you want to summarize a client's accounts?

1. *...you can quickly review their financial status.*
...
1. *...you can identify trends or issues for discussion.*
...
1. *...you can tailor recommendations to their situation.*
...
1. .*..you can show you understand their business and offer relevant insights.*
...
1. *...you can build a stronger relationship with them.*
...
| **Pattern** | **Examples** |
| --- | --- |
| so that `[audience]` can `[goal or outcome]`so `[audience]` can `[goal or outcome]` | <ul><li>…so that the support agent can quickly understand the issue</li><li>…so the manager has a comprehensive overview before the meeting</li></ul> |
| to `[infinitive verb phrase] ` | <ul><li>…to identify key customer pain points</li><li>…to generate a follow-up plan</li></ul> |
| in order to `[goal or outcome]` | <ul><li>…in order to prepare for the client meeting</li><li>…in order to encourage a response</li></ul> |
| for a `[goal or outcome]`for the purpose of `[goal or outcome]` | <ul><li>…for a new marketing campaign</li><li>…for the purpose of escalating the case appropriately</li><li>…for the purpose of keeping attendees up to date</li></ul> |
| The output will be used to `[goal or outcome]` | <ul><li>The output will be used to populate the opportunity record.</li><li>The output will be used to determine the next stage.</li></ul> |
...
| **Pattern** | **Examples** |
| --- | --- |
| **Your role is ____** | <ul><li>Your role is sales manager</li></ul> |
| **You are a _____** | <ul><li>You are a sales manager.</li><li>You are an AI agent</li><li>You’re a helpful assistant</li></ul> |
| **As a _____** | <ul><li>As a service advisor</li></ul> |
...
| **Sub-Element** | **Function** | **Pattern Examples** |
| --- | --- | --- |
| **Scope** | Narrow expertise or area of focus when specialized knowledge is needed.Provide boundaries for the agent’s expertise and the sources it relies on. | <ul><li>You have expertise in `[scope of knowledge]`</li><li>...with knowledge of `[scope of knowledge]`</li><li>...responsible for `[area of focus]`</li><li>...working in `[area of focus]`</li></ul> |
| **Job** | Describe key responsibilities or job functions briefly. | <ul><li>...to `[brief job description]`</li><li>...and your job is to `[brief job description]`</li><li>...focused on `[responsibilities]`</li></ul> |
| **Details** | Employ for prompt templates with recipients, like emails, and ground in data like name, title, and company. | <ul><li>...at a company called `[companyName]`...</li><li>...with an organization, `[companyName]`...</li><li>...and your name is `[name]`...</li><li>...working in `[industry]`...</li><li>You represent `[companyName]`...</li></ul> |
| **Relational Stance** | Utilize when the LLM is acting as an assistant to a human actor with a particular role or title. | <ul><li>...who assists a `[human role or title]`…</li><li>...assisting a `[human role or title]`...</li><li>...helping a `[human role or title]`...</li><li>You’re an assistant to a `[human role or title]`...</li></ul> |
| **Tone** | Establish behavioral cues for tone and personality that align with the role. | <ul><li>...and communicates in a `[style]` manner</li><li>You are a `[behavioral trait]` `[role or title]`</li><li>You prioritize `[style]` and `[style]` communication.</li></ul> |
...
| **Patterns** | **Examples** |
| --- | --- |
| …to `[actor]`…to `[adjective]` `[actor]` …to `[actor]` `[grounding data]` | <ul><li>…to new customers</li><li>…to the sales team</li><li>…to your prospect, {!$Input:Lead.FullName}</li></ul> |
| …shared with `[actor]` …shared with `[adjective]` `[actor]` | <ul><li>…shared with your prospect</li><li>…shared with a sales manager</li><li>…shared with an existing customer</li></ul> |
| …for `[actor]` …for `[adjective]` `[actor]` | <ul><li>…for a technical manager</li><li>…for the customer service team</li><li>…for our automated email marketing platform</li><li>…for internal review</li></ul> |
| …to be `[past participle]` by `[actor]` | <ul><li>…to be consumed by our analytics dashboard</li><li>…to be read by service representatives</li><li>…to be sent to a user new to the company</li></ul> |
| …for `[system]` expecting `[format]`…for `[system]` that expects `[format]` | <ul><li>…for a flow expecting JSON input</li><li>…for the external API that expects a string</li></ul> |
...
| --- | --- |
| **Channel** | <ul><li>[output] will be used in [channel]</li><li>[output] in [channel] </li></ul> |
| **Device Type** | <ul><li>[output] is intended for display on [device]</li><li>[output] will be used on [device].</li></ul> |
| **System** | <ul><li>The [output] will populate the [field name] field in a [object] record</li></ul> |
...
| **Types** | **Pattern Examples** |
| --- | --- |
| **Condition or Context** | <ul><li>This task is needed because [condition]</li><li>Current business context:</li></ul> |
| **Preceding Event** | <ul><li>Triggering event: [preceding event]</li><li>Your task is to [action] because [preceding event].</li><li>[preceding event], so you need to [action].</li><li>You’re tasked with [action] after [preceding event].</li></ul> |
...
| **Types** | **Pattern Examples** |
| --- | --- |
| **Time Frame** | <ul><li>...for `[time frame]`</li></ul> |
| **Real-Time** | <ul><li>This is a real-time response to a user query.</li></ul> |
| **Date-Bound** | <ul><li>...which must be no later than `[date]`...</li><li>...which can be earlier than `[date]`...</li></ul> |
