...
Custom radio buttons are created by applying the .slds-radio class to a  element. To remain accessible to all user agents, place an  with type="radio" inside the  element. The  is then visually hidden, and the styling is placed on a span with the .slds-radio_faux class. The styling of the span changes based on whether the radio button is selected or focused by using a pseudo-element. A second span with .slds-form-element__label contains the label text.

When a radio group is required, the  should receive the class .slds-is-required. The  should then get * added to the DOM for visual indication that the radio group is required.
