---
title: "Radio Group"
description: "Users can interact with radio buttons using a mouse by clicking the radio button or its label."
category: "component"
...
- When a radio group is required, the `<fieldset>` should receive the class `.slds-is-required`. The `<legend>` should then get `<abbr title="required">*</abbr>` added to the DOM for visual indication that the radio group is required.
...
- All radio groups marked as required with an * must have an accompanying legend.**Example:** * denotes a required field
