...
When the user clicks a primary action button, the [status text](/pattern/header) updates to reflect the action in progress, then updates again when the action is complete.
...
|**Order** | **Message** | **Time onScreen** | **Transition/Animation** | **Notes** |
| --- | --- | --- | --- | --- |
...
| 2, if failed | **Saved [time]** (Seconds, minutes, days, months, years) | Persistent | Opacity 0.25s ease-in | Update status message to the time of the last successful save. Include relevant warnings and errors in [*popovers*](/component/popover). |

### Activate/Deactivate

Activation and deactivation are significant milestones—use a [toast](/component/toast) to notify users of success or failure. When the action is in progress, update the status message to read “Activating...” or “Deactivating....” Use an animated spinner.
