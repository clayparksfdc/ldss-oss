...
| *                     Threaded Messaging UI that displays an avatar for all participants           *  | *                     Bubble Messaging UI that displays an avatar for all participants           *  |
| --- | --- |
|  |  |
...
An indicator for text-based channels or real-time transcription of audio that shows when a user is actively typing. There are some [ethical privacy concerns](/pattern/messaging-ui--message-entries) to be aware of when dealing with typing indicators.
...
An indication that shows if a message has been delivered, received, read, edited, or deleted. This can include [system messages](/pattern/messaging-ui--message-entries), such as when a user joins a conversation in addition to read receipts or even error messages related to sending or receiving messages.
