...
**Edit Icon**: Displayed if inline edit is enabled.Required Asterisk: Communicates that the input is required and triggers validation upon form submission.
...
To vertically stack  and  pairs, place slds-form-element_stacked on the div with the class of slds-form-element for optimal spacing.
...
To horizontally align a  and , use the slds-form-element_horizontal class on the div with the class of slds-form-element. The slds-form-element__label takes up 33% of the width, and the slds-form-element__control uses the remaining 66%.
...
The compound form should be implemented as a , where the fieldset label is implemented as a  element.

The  requires the class slds-form-element_compound. This class handles the layout and wrapping of the form elements that are grouped together.

The  element requires the class slds-form-element__legend. Legend elements can only accept a limited amount of CSS properties so this class is there to help manage its layout.
