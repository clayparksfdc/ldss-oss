---
title: "Tree Grid"
description: "Overall keyboard interaction should follow the same keyboard modal as the Advanced and Inline Edit Data Table with some additions"
category: "component"
...
- Overall keyboard interaction should follow the same keyboard modal as the [Advanced and Inline Edit Data Table](https://www.lightningdesignsystem.com/2e1ef8501/p/86f13a) with some additions<ul><li>Navigation mode is the default mode. The grid only has a single focusable element at any time and it is either the `tr` or the `td`</li><li>Actionable mode is enabled when the user presses the `Enter` key, where actionable elements become focusable in the cell</li><li>Actionable mode is exited when the user presses the `Escape` key, and the user is placed back into Navigation Mode on the last cell they were in</li></ul>
